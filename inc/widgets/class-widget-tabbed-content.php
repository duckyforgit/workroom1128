 ...
 * Class Widget Tabbed Content
 */
class Widget_Tabbed_Content extends WP_Widget {

	/**
	 * Widget_Tabbed_Content Constructor
	 *
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'tabbed-content', esc_url( home_url() ) . '/assets/js/tabbed-content.js', array( 'jquery' ), '1.0.0' );
		...
		<div class="tab-content" id="tabbed-posts">
			...
			for ( $i = 0; $i <= 3; $i++ ) :
				?>
					...
					if ( 0 === $i ) {
						?>
					<div class="tab-pane fade show active" id="<?php echo esc_html( $args['widget_id'] ); ?>-tabbed-<?php echo esc_html( $i ); ?>" role="tabpanel" aria-labelledby="<?php echo esc_html( $args['widget_id'] ); ?>-tab-<?php echo esc_html( $i ); ?>"><?php echo esc_html( $this->tab_content( $settings, $settings['tab_content'][ $i ] ) ); ?></div>
						<?php
					} else {
						?>
							<?php $id = $args['widget_id'] . '-tabbed-' . $i; ?>
							<?php $tabbedcontent = $this->tab_content( $settings, $settings['tab_content'][ $i ] ); ?>
							<div class="tab-pane fade" id="<?php echo esc_html( $id ); ?>" role="tabpanel" aria-labelledby="<?php echo esc_html( $id ); ?>"><?php echo esc_html( $tabbedcontent ); ?></div>
						<?php
					}
					?>
				<?php
		...
		switch ( $tabcontent ) :
			...
			<ul class="tabcontent-categories">
				...
				foreach ( $categories as $category ) {
					...
					// translators: 'category link' is a category %s.
					echo esc_html( '<li>' . $category_link . '&nbsp;&nbsp;(' );
					// Echo comment code echo '<p>' . sprintf( esc_html__( 'Description: %s', 'workroom1128' ), $category->description ) . '</p>';.
					// translators: '%s' is a category count.
					echo esc_html( '' . $category_count . ')</li>' );
				}
				?>
			</ul>
							<?php
				...
				<ul class="tabcontent-popular-posts posts-list">
					...
					if ( $posts_query->have_posts() ) :
						while ( $posts_query->have_posts() ) :
							...
							if ( true === $settings['thumbnails'] && has_post_thumbnail() ) :
								?>
							...
							<a href="<?php esc_url( the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ? get_the_title() : get_the_ID() ); ?>">
								...
								<?php
								// Display Date only on posts with thumbnails.
								if ( true === $settings['thumbnails'] && has_post_thumbnail() ) :
									?>
				...
				<ul class="tabcontent-comments comments-list">
					...
					if ( $comments ) :
						foreach ( (array) $comments as $comment ) :
							...
							<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>"><?php echo esc_html( get_the_title( $comment->comment_post_ID ) ); ?>
							</a> 
							<?php
					endforeach;
					endif;
				...
				if ( $related ) {
					...
					<ul class="tabcontent-related-posts tabbed-list">
						...
						if ( true === $settings['thumbnails'] && has_post_thumbnail() ) :
							...
							<span class="media-object">
								<span class="media-object-section">
									<a href="<?php echo esc_url( the_permalink( $post ) ); ?>" title="<?php echo esc_html( $post->post_title ); ?>">
						...
						?>
								<span class="media-object-section main-section">
									...
									<span class="meta-date meta-date">&nbsp;<?php echo esc_html( $time ); ?></span> </a>
									<!--// $post->the_time( get_option( 'date_format' ) );.-->
								</span>
							</span>
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'workroom1128' ); ?>
				<input class="widefat" id="<?php echo esc_html( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $settings['title'] ); ?>" />
			</label>
		</p>
		...
		for ( $i = 0; $i <= 3; $i++ ) :
			...
			<p style="border-bottom: 1px solid #eaeaea; padding: 10px; margin: 0;">
				...
				<label style="display: inline-block; min-width: 50px" for="<?php echo esc_html( $this->get_field_id( 'tab_content-' . $i ) ); ?>">
					<?php
					// translators: '%s' is a tab number.
					printf( esc_html__( 'Tab %s:', 'workroom1128' ), esc_html( $i + 1 ) );
					?>
				</label>

				<select id="<?php echo esc_html( $this->get_field_id( 'tab_content-' . $i ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'tab_content-' . $i ) ); ?>">
				...
				<label style="display: inline-block; min-width: 50px" for="<?php echo esc_html( $this->get_field_id( 'tab_titles-' . $i ) ); ?>">
					<?php esc_html_e( 'Title:', 'workroom1128' ); ?>
				</label>

				<input style="width: auto" id="<?php echo esc_html( $this->get_field_id( 'tab_titles-' . $i ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'tab_titles-' . $i ) ); ?>" type="text" value="<?php echo esc_attr( $settings['tab_titles'][ $i ] ); ?>" />
		...
		<strong><?php esc_html_e( 'Settings for Recent/Popular/Related Posts && Recent Comments', 'workroom1128' ); ?></strong>

		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of entries:', 'workroom1128' ); ?>
				<input id="<?php echo esc_html( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'number' ) ); ?>" type="text" value="<?php echo absint( $settings['number'] ); ?>" size="3" />
			</label>
		</p>

		<p>
			<label for="<?php echo esc_html( $this->get_field_id( 'thumbnails' ) ); ?>">
				<input class="checkbox" type="checkbox" <?php checked( $settings['thumbnails'] ); ?> id="<?php echo esc_html( $this->get_field_id( 'thumbnails' ) ); ?>" name="<?php echo esc_html( $this->get_field_name( 'thumbnails' ) ); ?>" />
